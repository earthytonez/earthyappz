...
export const getRegularPage = async (slug) => {
  ...
  const publishedResource = getSinglePages("content/resources");

  // filter by css
  const ssgData = publishedResource.filter((theme) =>
    theme.frontmatter.ssg.map((ssg) => slugify(ssg)).includes(slug)
  );
  //  filter by cms
  const cmsData = publishedResource.filter(
    (theme) =>
      theme.frontmatter.cms &&
      theme.frontmatter.cms.map((cms) => slugify(cms)).includes(slug)
  );
  //  filter by css
  const cssData = publishedResource.filter(
    (theme) =>
  ...
  // filter by archtype
  const categoryData = publishedResource.filter(
    (theme) =>
...
export const getRegularPageSlug = () => {
  ...
  const makingFile = getSinglePagesSlug(`content/making_music`);
  const learningFile = getSinglePagesSlug(`content/learning_music`);
  const appreciatingFile = getSinglePagesSlug(`content/appreciating_music`);
